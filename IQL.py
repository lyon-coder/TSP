...
class TSP_IQL(object):
    ...
    def calc_path_rewards(self, path, path_length):
        ...
        for fr, to in zip(path[:-1], path[1:]):
            ...
            reward_w = (self.mean_distance/path_length)**self.beta
            reward = reward_w*(self.mean_distance/dist)**self.alpha
            segment = (self.cities[0][fr],self.cities[1][fr], self.cities[0][to], self.cities[1][to])
            is_intersect = self.check_intersect(self.segment_set, segment)
            self.segment_set.append((self.cities[0][fr],self.cities[1][fr], self.cities[0][to], self.cities[1][to]))
            if is_intersect:
                reward -= (self.mean_distance/dist)**self.gamma
    ...
    async def train(self, num_epochs=1000, display=False):
        ...
        for epoch in range(num_epochs):
            ...
            if display:
                self.draw(epoch)
        Q_end = time.time()
        print("IQ Learning time is ", Q_end - Q_start)
    ...
    def check_intersect(self, segment_set, segment):
