...
class Solution:
    def __init__(self,X,start_node):
        ...
        self.array = [np.zeros(2**(len(self.X)-1), dtype='int64') for i in range(len(self.X))] #记录处于x节点，未经历M个节点时，矩阵储存x的下一步是M中哪个节点
...
def TSP_DP(cities):
    ...
    for i in range(n):
        for j in range(n):
            value = distance(cities[i],cities[j])
    ...
    plt.close()
    DP_end = time.time()
    print("DP time is ", DP_end - DP_start)
